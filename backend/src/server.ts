...
async function seedAdminUser() {
  ...
  const defaultPassword = 'Echovera@2026';
...
app.post('/api/auth/login', limit(authLimiter), asyncHandler(async (req, res) => {
  const schema = z.object({
    email: z.string().email().max(120),
    password: z.string().min(8).max(128),
    allowedRoles: z.array(z.enum(['owner', 'chef', 'waiter'])).optional(),
  });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) {
    return res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten() });
  ...
  if (parsed.data.allowedRoles && !parsed.data.allowedRoles.includes(user.role as any)) {
    return res.status(403).json({ success: false, message: `This account doesn't have access to this portal` });
  }
