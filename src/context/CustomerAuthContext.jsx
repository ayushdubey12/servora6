...
export function CustomerAuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      try {
        const stored = JSON.parse(localStorage.getItem('echovera-customer') || 'null');
  ...
  const persist = (data) => {
    localStorage.setItem('echovera-customer', JSON.stringify(data));
  ...
  const logout = async () => {
    localStorage.removeItem('echovera-customer');
    setCustomer(null);
    setToken(null);
  };
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem('echovera-customer', JSON.stringify({ customer: fullCustomer, token }));
      return fullCustomer;
    } catch {
      return null;
    }
