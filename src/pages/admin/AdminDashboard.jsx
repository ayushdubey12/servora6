    ...
    return (
      <div className="admin-page">
        ...
        <div className="admin-stats-grid">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="admin-stat-card shimmer" style={{ height: 100 }} />
          ))}
        </div>
      </div>
    );
          ...
          <div className="admin-card-body" style={{ padding: 0 }}>
            {(!stats?.recentOrders || stats.recentOrders.length === 0) ? (
              <div style={{ padding: 'var(--space-6)', textAlign: 'center' }}>
                <p className="body-md text-muted">No orders yet</p>
              </div>
            ) : (
        ...
        <div className="admin-card">
          ...
          <div className="admin-card-body">
            {(!stats?.restaurantRevenue || Object.keys(stats.restaurantRevenue).length === 0) ? (
              <p className="body-md text-muted" style={{ textAlign: 'center', padding: 'var(--space-6)' }}>
                No revenue data yet
              </p>
            ) : (
              <div className="revenue-list">
                {Object.entries(stats.restaurantRevenue)
                  .sort((a, b) => b[1] - a[1])
                  .map(([name, amount]) => (
